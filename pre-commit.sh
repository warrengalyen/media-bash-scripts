...
echo -e "* ${BOLD}Checking for unstashed changes:${NC}"
stash=0
...
if git diff-index --cached --quiet HEAD --; then
  ...
  echo -e "  - Stashed changes as: ${BOLD}${STASH_NAME}${NC}"
  if [ "$old_stash" = "$new_stash" ]; then
    echo -e "  - no changes, ${YELLOW}skipping tests${NC}"
    exit 0
  else
    stash=1
  fi
...
if [ "$status" -ne "0" ]; then
  echo -e "  - ${RED}failed:${NC} if you still want to commit use ${BOLD}'--no-verify'${NC}"
else
...
if [ "$stash" -eq 1 ]; then
  echo -e "* ${BOLD}Restoring working tree${NC}"
  if git reset --hard -q &&
    git stash apply --index -q &&
    git stash drop -q; then
    echo -e "  - ${GREEN}restored${NC} ${STASH_NAME}"
  else
    echo -e "  - ${RED}unable to revert stash command${NC}"
  fi
fi
